...
def log_validation(
    ...
    for tracker in accelerator.trackers:
        ...
        if tracker.name == "wandb":
            # Get the condition image from pipeline_args
            condition_image = pipeline_args.get("condition_image")
            
            # Create a list to store all wandb images (condition + generated)
            wandb_images = []
            
            # Add condition image first with a special caption
            if condition_image:
                wandb_images.append(
                    wandb.Image(condition_image, caption="Condition Image")
                )
...
def main(args):
    ...
    from src.jsonl_datasets import make_train_dataset, collate_fn, unpad_image
            ...
            if accelerator.sync_gradients:
                ...
                if accelerator.is_main_process:
                    ...
                    if args.validation_prompt is not None and global_step % args.validation_steps == 0:
                        ...
                        for prompt_idx, (validation_prompt, validation_image) in enumerate(zip(validation_prompts, validation_images)):
                            condition_image = unpad_image(Image.open(validation_image))
                            ...
                            for img_idx, img in enumerate(images):
                                img.save(os.path.join(save_folder, f"prompt{prompt_idx}_img{img_idx}.jpg"))
                            # save the condition image
                            condition_image.save(os.path.join(save_folder, f"prompt{prompt_idx}_condition.jpg"))
                        del pipeline
