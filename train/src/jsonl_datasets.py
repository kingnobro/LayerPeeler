...
def unpad_image(img, target_width=512, target_height=512):
...
def make_train_dataset(args, tokenizer, accelerator=None):
    ...
    def preprocess_train(examples):
        ...
        source_images = [unpad_image(Image.open(os.path.join(base_path, image)).convert("RGB")) 
                        for image in examples[source_column]]
        target_images = [unpad_image(Image.open(os.path.join(base_path, image)).convert("RGB")) 
