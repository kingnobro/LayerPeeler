...
def bbox_to_mask(box, H, W):
    ...
    x_min = int(math.floor((W * x_min)))
    x_max = int(math.ceil((W * x_max)))
    y_min = int(math.floor((H * y_min)))
    y_max = int(math.ceil((H * y_max)))
    position_mask = torch.zeros(H, W)
    position_mask[y_min: y_max, x_min: x_max] = 1
...
#     cond_latent_image_ids = prepare_latent_image_ids_2(original_height, original_width, device, dtype)
#     cond_latent_image_ids[..., 2] = cond_latent_image_ids[..., 2] + original_width // 2  # expand the width

#     latent_image_ids = latent_image_ids.reshape(
#             latent_image_id_height * latent_image_id_width, latent_image_id_channels
#         )
#     cond_latent_image_ids = cond_latent_image_ids.reshape(
#             latent_image_id_height * latent_image_id_width, latent_image_id_channels
#         )
...
class FluxPipeline(DiffusionPipeline, FluxLoraLoaderMixin, FromSingleFileMixin):
    ...
    def _get_t5_prompt_embeds(
        ...
        instance_text_index_list = []
        for p in prompt:
            region_prompt = p.split("$SEP$")
        ...
        prompt_embeds_list = [prompt_embeds[[0], :max_token_num, :]]
        begin_idx = max_token_num
        for i in range(1, len(region_prompt)):
            prompt_embeds_list.append(prompt_embeds[[i], :max_token_num, :].contiguous() )
            instance_text_index = torch.arange(begin_idx, begin_idx+max_token_num).int()
            instance_text_index_list.append(instance_text_index)
            begin_idx += max_token_num
        ...
        return prompt_embeds, instance_text_index_list, seq_len
    ...
    def encode_prompt(
        ...
        if prompt_embeds is None:
            ...
            prompt_embeds, instance_text_index_list, text_seq_len = self._get_t5_prompt_embeds(
                prompt=prompt_2,
                num_images_per_prompt=num_images_per_prompt,
        ...
        return prompt_embeds, pooled_prompt_embeds, text_ids, instance_text_index_list, text_seq_len
    ...
    def prepare_latents(
        ...
        timestep = torch.tensor([1000.], device=device)  # Should fix this hardcoded value
    ...
    def __call__(
        ...
        mask_list = [bbox_to_mask(bbox, height // 16, width // 16) for bbox in bbox_list]
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                self._joint_attention_kwargs['instance_text_index_list'] = instance_text_index_list
                self._joint_attention_kwargs['text_seq_len'] = text_seq_len
                self._joint_attention_kwargs['position_mask_list'] = mask_list

                self._joint_attention_kwargs['num_inference_steps'] = num_inference_steps
                self._joint_attention_kwargs['I2I_control_steps'] = i2i_control_steps
                self._joint_attention_kwargs['I2T_control_steps'] = i2t_control_steps
                self._joint_attention_kwargs['T2I_control_steps'] = t2i_control_steps
                self._joint_attention_kwargs['T2T_control_steps'] = t2t_control_steps
